  ...
  <li>
    ...
    <template v-else>
      ...
      <!-- Indented by one step per level rather than by the node's own depth: the rail is 262px
           wide, and five levels of a wider indent leave no room for a title. A nested list, so
           the depth is in the markup and not only in the margin. -->
      <ul v-if="open && node.children.length > 0" class="mt-1.5 ml-3 flex flex-col gap-1.5">
        <FolderRailNode
